...
- The User reports that the zombie-separation and Axe-separation mods conflict with each other: enabling Axe separation prevents the zombie-separation 2H animations from working. The User then reproduced the same incompatibility **with Gothic3_Animation_Behaviors absent**, establishing that the conflict exists between the two third-party separation mods themselves. This is not a failure or compatibility responsibility of Gothic3_Animation_Behaviors. Test our mod against each separation mod independently; do not treat the two third-party mods as a supported stack.
- No source change is justified.
