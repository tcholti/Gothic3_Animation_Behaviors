...
**Status:** CLOSED — PERMANENT PRODUCTION SUPPORT ACCEPTED THROUGH EV-329  
...
Production promotion is CLOSED/PASS.

Final acceptance chain:

```text
implementation 218c7ab73815a9afc46fc3ea5ac7246998bc2b1b
Normal Chat source review                                      PASS
Script_FrameCollisionBehaviorTest build                        PASS
Script_FrameCollisionTest build                                PASS
behavior-only deployment/startup                               PASS
diagnostic deployment/startup                                  PASS
EV-326 RIGHT->OFF->RIGHT permanent continuation                PASS
EV-327 BOTH with LEFT absent                                   NEGATIVE PASS
EV-328 ordinary true-Power protection                          PASS
EV-329 diagnostics-free behavior-only Goblin Sprint smoke      PASS
```

Pinned accepted behavior-only DLL SHA256:

`FE31030B35868EBF30B9644650991D8136625C4DBFD6263403350DB612BDB250`

Pinned diagnostic validation DLL SHA256:

`DAC9FFD8D4853947CEAD1F74569A6071E3C24AA716DC100064DBBC45086D7F3A`

Permanent support is now represented by `EquippedSprintCollision` and current authorities `DESIGN.md`, `COLLISION_REFERENCE.md`, `COLLISION_DIAGNOSTICS.md` and `COLLISION_TEST_PLAN.md`.

The temporary `EquippedSprintProbe` remains deleted.

Deferred non-behavior cleanup:
- diagnostic startup `BehaviorCore:` descriptive banner does not yet list `EquippedSprintCollision`;
- do not reinterpret this as a behavior defect;
- fix later as a bounded diagnostics-metadata cleanup, with its own rebuilt binary if desired.

Phase 4 standalone non-weapon creature regression resumes after this closure.
