...
**Status:** CLOSED / OBSERVATION PASS — CANDIDATE CALLBACKS RULED OUT — EV-351  
**Opened:** 2026-09-20  
...
## Runtime closure — EV-351

Local runtime:
- both twins built successfully;
- sole-live diagnostic deployment PASS;
- built/live SHA256 `F6EB69B1AFAFCB4C8F99FBE4C5CFDE1DC411FA074F109E7F5515D068E76096DD`;
- startup / hook installation / unload clean.

Six factual Gargoyle raw8 Power frame-3 invocations:
- all six consumed early raw8 timing opportunity;
- C1 14, 24 and 33 reached exact `Game+0x16E348` native contact-path observation;
- C1 4, 21 and 36 did not;
- `CORE RAW8_CONTACT_GATE` = 0;
- `CORE RAW8_CONTACT_TARGET` = 0.

Therefore exact raw8 `CanBeActivatedNow` / `TriggerTarget` callbacks are ruled out as the contact-consumption boundary on this tested route.

Direct invocation-level inherited visited-state behavior remains unresolved and is intentionally left for the next bounded question.
