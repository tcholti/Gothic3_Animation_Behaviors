...
**Status:** CLOSED — IMPLEMENTED / FINAL SOURCE FROZEN / FOCUSED ACCEPTANCE COMPLETE  
**Opened:** 2026-09-20  
...
Current production source is `f1f5d2aad3edc3564a9a8b40541840b94f8fa903`. Both twins rebuilt successfully. Diagnostic deployment/startup and targeted post-cleanup re-smoke passed at EV-357; diagnostics-free behavior parity smoke passed at EV-358 with positive mod discriminators. The source is now **frozen** for the focused permanent raw8 acceptance matrix. No further source cleanup is authorized absent a newly demonstrated contradiction.
...
Permanent raw8 persistent-opportunity production implementation is complete and accepted.

Final frozen source:
`f1f5d2aad3edc3564a9a8b40541840b94f8fa903`

Final focused acceptance:
- EV-357–EV-358: post-cleanup diagnostic + behavior parity PASS;
- EV-359: human Normal/Power, later-FIST reopening and ordinary finalization PASS;
- EV-360: Gargoyle far/no-contact, reaction replacement and Sprint transport PASS;
- EV-361: native unmarked human target-directed control PASS;
- EV-364: marked pending -> generation replacement -> genuinely unmarked native fallback PASS.

All ten §14 requirements in `docs/COLLISION_RAW8_PRODUCTION_ARCHITECTURE.md` are PASS.

No source contradiction was exposed by focused acceptance. Further raw8 source changes require newly demonstrated contradictory evidence and a newly frozen bounded responsibility.

Next project responsibility is the comprehensive frozen-source standalone collision regression in `docs/COLLISION_TEST_PLAN.md` §3.8, followed only later by New Balance + AttackCollision compatibility.
