...
Implemented in `9789254866bb47dcbadb145aeb9831c7c0e4e97f` with parent `dd3b5efd22c3403f18a942dac6040da240441648`.

Independent Normal Chat static review: **PASS**.

Verified:
- exact seven-file source scope;
- bridge ordering: native AISetState original -> existing raw8 finalization -> FrameCollisionMarkers metadata retirement -> EquippedSprintCollision binding retirement -> PhysicalFistCollision execution retirement -> existing CollisionLifecycleGuard finalization;
- each owner erases only when actor key exists **and** stored C1 generation equals the captured finalized generation;
- replacement-generation records are therefore preserved;
- no marker-owned window retirement;
- no collision-group, latch, source-operation, diagnostic, hook, CMake or lifecycle-policy change;
- native original-call ordering and existing raw8/generic lifecycle logic remain unchanged.

Runtime/build validation remains intentionally deferred until the second accepted cleanup is implemented and independently reviewed.
