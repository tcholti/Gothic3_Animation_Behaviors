...
**Status:** CLOSED / CAUSAL INTERVENTION PASS — EV-348
**Opened:** 2026-09-20
...
EV-348 closed the probe.

The controlled frame-3 Gargoyle Power run produced:
- 4 exact synthetic hits -> no probe rearm;
- 9 exact synthetic misses -> 9 confirmed post-miss latch rearms 1 -> 0;
- 7 of those 9 rearms -> later exact native raw8 damage before same-C1 finalization;
- 2 rearms -> no later damage in that C1.

Conclusion: latch-only post-miss rearm is causally sufficient to restore a later native raw8 opportunity on the tested Power route. The probe did not rearm timing permission and therefore does not prove continuous eligibility from marker time to Gothic's later native threshold.
