...
**Status:** CLOSED — DIAGNOSTIC CAUSAL CASE ACCEPTED / PROMOTION APPROVED  
...
The causal research question is closed through EV-315 and EV-320–EV-324.

Normal Chat production-promotion review concluded that the evidence is sufficient to promote the proven equipped-Sprint mechanism into permanent behavior, but **not** by compiling the temporary diagnostic `EquippedSprintProbe` into production.

Permanent implementation must:
- extract the proven policy/state into a behavior module;
- preserve EngineBridge as transport-only;
- keep generic marker semantics in `FrameCollisionMarkers`;
- remain mechanically independent of diagnostics;
- preserve missing-source fail-closed behavior and ordinary true-Power isolation;
- delete the temporary probe dependency after extraction.
