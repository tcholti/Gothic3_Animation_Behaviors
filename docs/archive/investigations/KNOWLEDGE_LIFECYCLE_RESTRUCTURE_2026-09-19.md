...
The restructure completed its review gates on the temporary branch.

Verified before promotion:

- current reference / architecture / ADR / evidence / temporary-work roles are separated;
- closed raw55 probes, implementation contracts, audit material and superseded plans moved to cold archive;
- closed evidence ledgers moved to `docs/archive/evidence/`; one active EV-309+ ledger remains;
- `COLLISION_REFERENCE.md` and existing stable authorities carry promoted reusable conclusions;
- `EVIDENCE_INDEX.md` is compact routing rather than a second knowledge narrative;
- root `README.md` is the unique ordinary/interrupted startup front door;
- lifecycle rules are routed consistently through `KNOWLEDGE_MAINTENANCE.md`;
- `tools/knowledge/validate_knowledge_state.py` plus GitHub Actions enforce structural and selected semantic invariants;
- manual semantic scan found no current references to superseded collision plan/probe filenames and no remaining evidence-first ordinary retrieval instruction;
- compare against base SHA `a0edcf5556c25ea45e619c52d272d33a8b9cd326` contains documentation/documentation-automation changes only;
- GitHub Actions knowledge-state validation passed after the strengthened validator and pipeline cleanup.

This record is archived before promotion so no closed restructure task remains in `docs/work/active/`.
