...
**Status:** CLOSED / CAUSAL OBSERVATION PASS — EV-347  
**Opened:** 2026-09-20  
...
EV-347 closed the probe.

The controlled Gargoyle frame-3 fixture produced four exact `SyntheticApplied=1` observations:
- two misses with no same-invocation `Game+0x16E348` raw8 damage;
- two hits with exact same-invocation `Game+0x16E348` raw8 damage;
- all four returned with `SPU+0x164 = 1`.

Conclusion: the tested native raw8 path consumes/closes the latch after the attempt, not only after successful damage.
