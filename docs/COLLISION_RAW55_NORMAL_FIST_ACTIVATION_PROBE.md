# Raw55 PhysicalFist Normal Authored-FIST Activation Probe
...
Answer one causal question after EV-278 and before any permanent raw55 implementation is designed:

> With factual marked raw55 Normal early `Item_Equipped/group5 -> Item_Attack/group7` still selectively suppressed and the original `_AI_Attack` callback otherwise running normally, can the authored `G3AB_COL_FIST` event itself open the exact RIGHT PhysicalFist/raw55 source `5 -> 7` at authored timing, with Gothic retaining ownership of contact/damage/cleanup?
...
Governing evidence: `EVIDENCE_LEDGER_274_ONWARD.md` EV-277 through EV-278.

---

## 1. Evidence Boundary

EV-277 established Normal compound callback ownership:
...
EV-278 establishes separability. Seven factual Troll Normal C1s independently suppress the exact scoped native `5 -> 7` while the immediately enclosing callback still returns:
...
Completed representative Normal executions remain group5 through authored FIST at `StatePosition=1`; stable marker processing reports `UNSUPPORTED_MISSING_SOURCE`; no alternate raw55 opening appears; no Normal damage occurs; and completed C1s finalize with no outstanding offense or repair.

Therefore early gating is no longer the open question. The next unknown is whether the exact raw55 source can be opened at authored Normal FIST time while preserving native progression and cleanup.

---

## 2. Single Probe Question

For an exact marked factual Normal raw55 execution that has already demonstrated EV-278 selective early-group suppression in the same C1:

```text
authored G3AB_COL_FIST occurs
+ factual Family=NORMAL / Action=1 Hit
+ native StatePosition is 1
+ exact RIGHT PhysicalFist/raw55 source is still group5
+ same actor + RIGHT source + C1 as the prior Normal suppression proof
...
`PhysicalFistProbe` must record a bounded per-actor Normal suppression proof only when the EV-278 selective suppression actually occurs.
...
same actor + RIGHT source + C1 as EV-278 suppression proof
...
The FIST-time request occurs outside the active `_AI_Attack` callback scope, so the EV-278 selective early-suppression rule must not suppress it.
...
`PhysicalFistProbe` owns all temporary Normal research state/policy:

```text
Normal early-suppression proof
same actor/source/C1 validation
exact Normal/FIST eligibility
one-shot marker-time activation decision
exact RIGHT raw55 group request
compact probe logging
```

Do not move Normal policy/state into `EngineBridge`.
...
```text
CORE RAW55_NORMAL_FIST_ACTIVATION_PROBE
```
...
closed true-Power EV-275/EV-276 behavior and research state
Normal original callback exactly once
EV-278 Normal early selective suppression
...
Sprint authored-FIST activation
...
```text
EV-278 Normal selective suppression remains unchanged
original Normal callback still executes exactly once
...
Action1 / Family NORMAL / Hit eligibility required
...
```text
one or more factual Family=NORMAL / Action1 Troll executions
incidental Quick/Power/Sprint traffic is harmless
User does not need to identify the family visually
```

Correlate each Normal C1:
...
Physical authored activation is proven, but activation alone is insufficient. The next question becomes a separate contact/rearm primitive: whether one exact `TouchDamage.ClearTriggeredList()` at Normal FIST time is required. Do not combine that intervention into this probe.
...
After one adequate runtime capture, Normal Chat promotes the evidence and freezes only the next smallest causal responsibility. Do not implement permanent raw55 behavior or Sprint redesign in the same task.
