...
| EV-078 | Corrected FrameCollision v0.9 source reads raw UseType through `Entity.GetInstance()` and the underlying `eCEntity::GetUseType()`, then skips `SetCollisionGroup(Item_Attack)` only for Fist/PhysicalFist while keeping unconditional `ClearTriggeredList` and all non-Fist behavior. | CONFIRMED AS CORRECTED SOURCE IMPLEMENTATION; BUILD/RUNTIME PENDING | commit `9b4a73c`; pinned SDK declarations + static source review | Rebuild before installation; causal Fist conclusion still requires controlled contact tests. |
