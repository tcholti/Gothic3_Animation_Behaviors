...
| EV-077 | FrameCollision v0.9 source skips `SetCollisionGroup(Item_Attack)` only when the resolved right-slot source raw UseType is Fist or PhysicalFist, while preserving marker ownership, native suppression, source resolution, unconditional `ClearTriggeredList`, and the proven non-Fist path. | CONFIRMED AS SOURCE IMPLEMENTATION; BUILD/RUNTIME PENDING | commit `89f36d8`; static source review | Build and run marked Fist limb-contact tests; do not promote the causal conclusion until damage behavior is observed. |
