...
DECLARE_SCRIPT_CALLBACK(OnAI_PowerAttack_FrameCollisionTest)
{
    ...
    if (EvaluateAttackCallback(SelfEntity, family, a_pSPU, &ownership))
        return GETrue;
    if (EquippedSprintCollision::ShouldSuppressNativeCallback(
...
{
    eCResourceDataEntity *const routedResource =
        AttackMotionRouting::QueryOptionalSubstitution(
            GetCombatMoveFactualAction(a_pSPU), a_pAnimationAdmin,
            a_ResourceName, a_CacheBehavior);
    if (routedResource != nullptr)
    ...
    bCMatrix const *a_pMatrix, GEBool a_bUnknown)
{
    ...
    if (!raw55MarkerOwned)
    {
        bool const equippedSprintAuthorized =
            EquippedSprintCollision::AuthorizeGenericEquippedMarker(
                actor, markerOpcode);
...
void FrameCollision::EngineBridge::InstallHooks()
{
    ...
    Hook_Raw8FistTimingGateGetPlayTime
        .Prepare(RVA_Game(0x16E180),
                 &Raw8FistTimingGateGetPlayTime_FrameCollisionTest)
