...
| `archive/2026-08-22_framecollision_v0.9_player_fist_native_left_hand.log` | 2026-08-22 | Native-motion left-hand baseline damaged the target while `PC_Hero` used marked frame-controlled ownership. Eight accepted frame-3 markers resolved raw Fist UseType 8, skipped `SetCollisionGroup(Item_Attack)`, kept collision group 0 -> 0, and cleared the triggered list. The session also contained an unmarked legacy/native Boar Fist attack path, so passive targets are preferred for later variants. | `docs/EVIDENCE_LEDGER.md` EV-080; `docs/HANDOFF.md`; `docs/RESEARCH_MAP.md`; `docs/ANIMATION_CATALOG.md` |
